  ...
  <?php } ?>
        ...
        <script type="text/javascript">
            ...
            function clear_cart() {
                ...
                if (result) {
                    window.location = "<?php echo base_url(); ?>user_cart/remove/all";
                } else {
			 ...
			 <?php if(!empty($cart_check)) { ?>
                ...
                <table id="table" border="0" cellpadding="5px" cellspacing="1px" align="center">
                  ...
                  if ($cart = $this->cart->contents()): ?>
                    ...
                    echo form_open('user_cart/update_cart');
                    $grand_total = 0;
                    $i = 1;
                    ...
                    foreach ($cart as $item):
                      ...
                      <?php echo $row->description; ?>
                        ...
                        <?php $grand_total = $grand_total + $item['subtotal']; ?>
                            ...
                            <td>
							<a href="<?php echo base_url(); ?>user_cart/remove/<?php echo $item['rowid']; ?>">
...
<div class="container--">
    ...
    <div class="col-md-3-- col-sm-3--">
        <div id="bill_info--">
            ...
            <form name="billing" method="post" action="<?php echo base_url() . 'user_cart/save_order' ?>" >
                <input type="hidden" name="command" />
                <div align="center" class="border-table--" style="border: 1px #7f7f7f solid">
                    <p align="center">
					<div id = "heading" style="font: 18px Verdana">
						<p>BILLING INFO</p>
					</div>                    
                    </p>
                    <div class="table-responsive">
		<?php
			...
			{	
		?>	      
                    <table border="0" cellpadding="5px" width="37%">
                    <?php $discount = ($grand_total * 0.025); ?>
                    <?php $total_discount = ($grand_total - $discount); ?>                       
                        <?php if ($grand_total > 10000) { ?>
                        <tr><td>Order Total :</td><td><strong>$<?php echo number_format($grand_total,2); ?></strong></td></tr>
                        <tr><td>Discount :</td><td><strong>$<?php echo number_format($discount,2); ?></strong></td></tr>
                        <tr><td>Total Payment :</td><td><strong>$<?php echo number_format($total_discount,2); ?></strong></td></tr>
                        <input type="hidden" name="total" required="" value="<?php echo $grand_total ?>" />
                        <input type="hidden" name="disc" required="" value="<?php echo $discount ?>" />
                        <input type="hidden" name="pay" required="" value="<?php echo $total_discount ?>" />                        
                        <?php }else{ ?>
                        <tr><td>Total Payment:</td><td><strong>$<?php echo number_format($grand_total,2); ?></strong></td></tr>
                        <input type="hidden" name="pay" required="" value="<?php echo $grand_total ?>" />
                        <?php } ?>

                        <tr><td width="100%">Your Name:</td>
							<td><input type="text" name="name" required="" style="width: 250px" value="<?php echo $row->first_name. " ".$row->last_name; //echo $email ?>" disabled="disabled"/></td><input type="hidden" name="name" required="" style="width: 250px" value="<?php echo $row->first_name. " ".$row->last_name; //echo $email ?>" />
						</tr>
                        <tr><td>Address:</td><td><textarea class="span4" name="address" rows="0" style="width: 250px"></textarea></td></tr>
                        <tr><td>Email:</td><td><input type="text" name="email" disabled="disabled" required="" style="width: 250px" value="<?php echo $row->email_address; //echo $email ?>" /></td></tr><input type="hidden" name="email" required="" style="width: 250px" value="<?php echo $row->email_address; //echo $email ?>" />

		<?php
			/*
			foreach ($cart as $item):
			$this->db->select('*');
			//$this->db->group_by("email_address"); 
			$this->db->where('serial', $item['id']);
			$query = $this->db->get('property');
			//$this->db->group_by("email_address"); 
		...
		?>
                 ...
                 <!-- <input type="text" name="email2" required="" style="width: 250px" value="<?php //echo $row->email_address; //echo $email ?>" /> -->
        <?php //} ?> 
        <?php //endforeach; ?>               
                        <tr><td>Phone:</td><td><input type="text" name="phone"  required="" style="width: 250px" /></td></tr>
                        <tr><td><?php
                        // This button for redirect main page.
                        //echo "<a class ='btn btn-default' id='back' href=" . base_url() . "index.php/shopping>Back to order</a>";  ?>
                            </td><td><br><a href="<?php echo base_url() ?>admin/property">
							<input type="button" class ='btn btn-default' value="Back" /></a>
                            <input type="submit" class ='btn btn-info' value="Place Order" /></td></tr> 
